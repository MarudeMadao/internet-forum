...
<html lang = "ja">
    ...
    <body>
        ...
        <?php
            ...
            $query = mysqli_query($mysqli, "select * from agenda") or die("検索に失敗しました");

            $flag = 0;
            while($data = mysqli_fetch_array($query)){
                if(!empty($title) and strcmp($data["title"], $title) == 0){
                    echo "すでにスレッド番号" . $data["thread"] . "で同名のタイトルが存在します。
                        <br>同名でのスレッド作成はできません。";
                    $flag = 1;
                }
            }
            if($flag == 0 and !empty($title)){
                $query = mysqli_query($mysqli, "insert into agenda (title, created) values ('$title', now())") or die ("スレッドの作成に失敗しました。");
            }

            $query = mysqli_query($mysqli, "select * from agenda order by thread desc") or die("検索に失敗しました");
